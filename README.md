...
Omit `--user` to inspect only the project. The official [GitHub release](https://github.com/tangericm/skillcrit/releases/tag/v0.5.2)
includes the same built package published on npm, checksums and verification evidence.
For a disposable installation,
checksum verification and a first audit, follow the [getting-started guide](docs/pilot-guide.md).
...
Install from npm as shown above, or build this checkout with `npm ci` and `npm run build`.
...
before invoking them. npm `latest` tracks the stable release; `next` is reserved
for release candidates. Pin the version and commit the lockfile for reproducible CI.

**Let your agent run it** — installs `skills/skillcrit` as an agent skill:

```bash
# First install the CLI from the built checkout or npm package.
skillcrit --version
...
`npm install --save-dev --save-exact skillcrit@0.5.2 --ignore-scripts`
